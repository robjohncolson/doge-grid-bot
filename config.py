...
# Signal Digest controls (Layer 1 diagnostics + optional Layer 2 interpretation).
DIGEST_ENABLED: bool = _env("DIGEST_ENABLED", True, bool)
DIGEST_INTERPRETATION_ENABLED: bool = _env("DIGEST_INTERPRETATION_ENABLED", True, bool)
DIGEST_INTERPRETATION_INTERVAL_SEC: float = _env("DIGEST_INTERPRETATION_INTERVAL_SEC", 600.0, float)
DIGEST_INTERPRETATION_DEBOUNCE_SEC: float = _env("DIGEST_INTERPRETATION_DEBOUNCE_SEC", 120.0, float)
DIGEST_EMA_AMBER_THRESHOLD: float = _env("DIGEST_EMA_AMBER_THRESHOLD", 0.003, float)
DIGEST_EMA_RED_THRESHOLD: float = _env("DIGEST_EMA_RED_THRESHOLD", 0.01, float)
DIGEST_RSI_AMBER_ZONE: float = _env("DIGEST_RSI_AMBER_ZONE", 0.2, float)
DIGEST_RSI_RED_ZONE: float = _env("DIGEST_RSI_RED_ZONE", 0.4, float)
DIGEST_AGE_AMBER_PCT: float = _env("DIGEST_AGE_AMBER_PCT", 30.0, float)
DIGEST_AGE_RED_PCT: float = _env("DIGEST_AGE_RED_PCT", 60.0, float)
DIGEST_EXIT_AMBER_PCT: float = _env("DIGEST_EXIT_AMBER_PCT", 3.0, float)
DIGEST_EXIT_RED_PCT: float = _env("DIGEST_EXIT_RED_PCT", 8.0, float)
DIGEST_HEADROOM_AMBER: int = _env("DIGEST_HEADROOM_AMBER", 50, int)
DIGEST_HEADROOM_RED: int = _env("DIGEST_HEADROOM_RED", 20, int)
...
# Ranger mode controls (standalone DOGE sell->buy micro-cyclers).
RANGER_ENABLED: bool = _env("RANGER_ENABLED", False, bool)
RANGER_MAX_SLOTS: int = _env("RANGER_MAX_SLOTS", 3, int)
if RANGER_MAX_SLOTS < 1:
    RANGER_MAX_SLOTS = 1
RANGER_ORDER_SIZE_USD: float = _env("RANGER_ORDER_SIZE_USD", 2.0, float)
if RANGER_ORDER_SIZE_USD <= 0:
    RANGER_ORDER_SIZE_USD = 2.0
RANGER_ENTRY_PCT: float = _env("RANGER_ENTRY_PCT", 0.15, float)
if RANGER_ENTRY_PCT <= 0:
    RANGER_ENTRY_PCT = 0.15
RANGER_PROFIT_PCT: float = _env("RANGER_PROFIT_PCT", 1.20, float)
if RANGER_PROFIT_PCT <= 0:
    RANGER_PROFIT_PCT = 1.20
RANGER_ENTRY_TIMEOUT_SEC: int = _env("RANGER_ENTRY_TIMEOUT_SEC", 300, int)
if RANGER_ENTRY_TIMEOUT_SEC < 60:
    RANGER_ENTRY_TIMEOUT_SEC = 60
RANGER_EXIT_TIMEOUT_SEC: int = _env("RANGER_EXIT_TIMEOUT_SEC", 1350, int)
if RANGER_EXIT_TIMEOUT_SEC < RANGER_ENTRY_TIMEOUT_SEC:
    RANGER_EXIT_TIMEOUT_SEC = RANGER_ENTRY_TIMEOUT_SEC
RANGER_COOLDOWN_SEC: int = _env("RANGER_COOLDOWN_SEC", 30, int)
if RANGER_COOLDOWN_SEC < 0:
    RANGER_COOLDOWN_SEC = 0
RANGER_MIN_HEADROOM: int = _env("RANGER_MIN_HEADROOM", 10, int)
if RANGER_MIN_HEADROOM < 0:
    RANGER_MIN_HEADROOM = 0
RANGER_DOGE_RESERVE: float = _env("RANGER_DOGE_RESERVE", 100.0, float)
if RANGER_DOGE_RESERVE < 0:
    RANGER_DOGE_RESERVE = 0.0
